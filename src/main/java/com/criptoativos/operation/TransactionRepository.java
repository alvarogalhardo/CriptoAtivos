...
public interface TransactionRepository
     ...
     *
     * <p>A single query with {@code (:param is null or ...)} branches would be simpler to write, but
     * Postgres cannot infer the type of a null bind parameter (it fails with {@code function
    ...
    final class Specs {
        ...
        public static Specification<Transaction> ofType(TransactionType type) {
            return type == null ? null : (root, query, cb) -> cb.equal(root.get("transactionType"), type);
        }
        ...
        public static Specification<Transaction> forSymbol(String symbol) {
            return symbol == null || symbol.isBlank()
                    ? null
                    : (root, query, cb) ->
                            cb.equal(cb.upper(root.get("asset").get("symbol")), symbol.toUpperCase());
        }
